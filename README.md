...
**Reference faces (facebank):** Put one folder per person under `facebank/` with their photos, then run a demo with `-u` once to build embeddings. See `facebank/README.md` for details.
...
* over cam
  ...
  python cam_demo.py
  ```
  (On Wayland: `QT_QPA_PLATFORM=xcb python cam_demo.py`)
* over video file
  ```
  python Video_demo.py --video /path/to/video.mp4 [--output out.avi]
  ```
  Build/refresh facebank first: add `-u` to either command (e.g. `python cam_demo.py -u`).
4. Instruction 
  ```
...
3. Train MobileFaceNet
  ...
  python Train.py
  ```
  **Optional – faster training with NVIDIA DALI (GPU decode):** Requires **Python 3.8–3.12** (not 3.14). Install DALI then run with `--use_dali`:
  ```
  uv pip install nvidia-dali-cuda130   # CUDA 13; or nvidia-dali-cuda120 / nvidia-dali-cuda110
  uv run Train.py --dataset CASIA --use_dali
  ```
  If you're on Python 3.14, use a 3.11/3.12 venv for DALI, or train without `--use_dali`. Check CUDA: `nvidia-smi`.
4. Instruction 
  ```
