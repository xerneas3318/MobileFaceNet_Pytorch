...
if __name__ == '__main__':
    ...
    parser.add_argument("-c", "--score", help="show match similarity 0-100 (higher = better match)", action="store_true", default=False)
    ...
    "Minimum face to be detected. derease to increase accuracy. Increase to increase speed",
                        default=20, type=int)
    parser.add_argument('--video', '-v', dest='video', default='images/TheBigBangTheory.mp4',
                        help='path to input video file')
    parser.add_argument('--output', '-o', dest='output', default='output.avi',
    ...
    if not cap.isOpened():
        print('Error: could not open video {!r}'.format(args.video))
        sys.exit(1)
    ...
    while True:
        ...
        if frame is None or frame.size == 0:
            continue
        if out is None:
            out = cv2.VideoWriter(args.output, cv2.VideoWriter_fourcc(*'MJPG'), 20.0,
                                  (frame.shape[1], frame.shape[0]), isColor=True)
        try:
            start_time = time.time()
            input_img = resize_image(frame, args.scale)
            bboxes, landmarks = create_mtcnn_net(input_img, args.mini_face, device,
                                                p_model_path='MTCNN/weights/pnet_Weights',
                                                r_model_path='MTCNN/weights/rnet_Weights',
                                                o_model_path='MTCNN/weights/onet_Weights')

            # Ensure numpy; MTCNN can return empty landmarks (0,) when O-Net filters all, or bboxes (n,5) with landmarks (0,) if O-Net wasn't run
            ...
            if len(faces) > 0 and len(faces) == len(bboxes):
                if args.detect_only:
                    ...
                    for i, b in enumerate(bboxes):
                        ...
                        draw.rectangle([(b[0], b[1]), (b[2], b[3])], outline=box_color, width=2)
                        if args.score:
